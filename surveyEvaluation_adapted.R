...
# read file
surveyData <- read.csv("SurveyExport_adapted.csv", na.strings = c("", " "))

# common data
isInvertedNfc <- c(TRUE, FALSE, FALSE, TRUE) # points out which values must get inverted
isInvertedBigFive <- logical(length = 16)
isInvertedBigFive[c(3,6,8,15)] <- TRUE
attributesBigFive <- c("O","C","E","A","N")
selectionMatrixBigFive <- matrix(c(4,1,2,3,5, 10,8,6,7,11, 14,12,9,13,15, 16,NA,NA,NA,NA), ncol = 4)
...
personalityCount <- getPersonalityCount(select(surveyData, X1.personality_questionnaire:X16.personality_questionnaire))
