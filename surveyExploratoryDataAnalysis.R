...
a <- select(surveyDataCombined, dateSubmitted, group, personality, opinion_changed) %>% mutate(op = cut(opinion_changed, breaks = c(-3,0,2))) %>% mutate(timeSubmitted = as.numeric(hms(format(as.POSIXct(dateSubmitted, format = "%Y-%m-%d %H:%M:%S"), "%H:%M:%S")))/3600)
