...
removeOutliers <- function(data, timeToFinishThreshold = 120, timeOnTextThreshold = 25, timeToFinishUpperThreshold = Inf) {
  ...
  cleanedData <- filter(cleanedData, timeToFinish >= timeToFinishThreshold & timeToFinish <= timeToFinishUpperThreshold)
